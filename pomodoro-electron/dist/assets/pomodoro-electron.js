  ...
  exports['default'] = _ember['default'].Component.extend({
    ...
    setLeftPanelModel: function setLeftPanelModel(item) {
      if (this.get('listMode') === 'tasks') {
        ...
        this.set('dynComp.tags', this.get('model.tags'));
        this.set('dynComp.');
        this.set('dynComp.model', item.name ? item : this.get('newTask'));
        this.set('dynComp.saveAction', item.name ? 'editTask' : 'createTask');
...
if (!runningTests) {
  require("pomodoro-electron/app")["default"].create({"name":"pomodoro-electron","version":"0.0.0+a865f3b0"});
}
